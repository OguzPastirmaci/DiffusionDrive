...
def initialize_ray(
    ...
    # H200 GPU support - enable if available
    number_of_gpus_per_node = torch.cuda.device_count() if torch.cuda.is_available() else 0
    if number_of_gpus_per_node:
        logger.info(f"Using {number_of_gpus_per_node} H200 GPUs in ray")
    else:
        logger.info("No GPUs available for ray")
