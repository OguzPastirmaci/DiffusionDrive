...
class TransfuserConfig:
    ...
    # Transformer (H200 optimized - increased capacity)
    tf_d_model: int = 512  # increased from 256 for H200
    tf_d_ffn: int = 2048   # increased from 1024 for H200
    tf_num_layers: int = 4 # increased from 3 for H200
    tf_num_head: int = 16  # increased from 8 for H200
    ...
    # optimizer (H200 optimized)
    weight_decay: float = 5e-5  # reduced for better convergence on H200
    lr_steps = [50, 80]  # adjusted learning rate schedule for H200
    optimizer_type = "AdamW"
    scheduler_type = "MultiStepLR"
    cfg_lr_mult = 0.3  # reduced for better stability on H200
    opt_paramwise_cfg = {
        "name":{
            "image_encoder":{
                "lr_mult": cfg_lr_mult
